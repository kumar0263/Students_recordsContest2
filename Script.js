const studentData = [{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
function renderTable(data) {
    ...
    studentData.forEach(student => {
        ...
        nameCell.innerHTML = `<img src="${student.img_src}" alt="${student.first_name} ${student.last_name}"> ${student.first_name} ${student.last_name}`;
        ...
        passingCell.textContent = student.passing ? "true" : "false";
...
searchInput.addEventListener('input', () => {
    ...
    renderTable(studentData.filter(student =>
...
sortAZ.addEventListener('click', () => {
    renderTable(studentData.sort((a, b) => (a.first_name + ' ' + a.last_name).localeCompare(b.first_name + ' ' + b.last_name)));
});

sortZA.addEventListener('click', () => {
    renderTable(studentData.sort((a, b) => (b.first_name + ' ' + b.last_name).localeCompare(a.first_name + ' ' + a.last_name)));
});

sortMarks.addEventListener('click', () => {
    renderTable(studentData.sort((a, b) => a.marks - b.marks));
});

sortPassing.addEventListener('click', () => {
    renderTable(studentData.filter(student => student.passing === true));
});
sortClass.addEventListener('click', () => {
    renderTable(studentData.sort((a, b) => a.class - b.class));
});
...
sortGender.addEventListener('click', () => {
    ...
    const genderTableContainer = document.createElement('div');
    genderTableContainer.className = 'gender-table-container';
    
    const femaleTable = document.createElement('table');
    femaleTable.innerHTML = '<thead><tr><th>Image</th><th>Name</th><th>Email</th><th>Status</th></tr></thead>';
    const femaleTableBody = document.createElement('tbody');
    renderTable(femaleStudents, femaleTableBody);
    femaleTable.appendChild(femaleTableBody);
    
    const maleTable = document.createElement('table');
    maleTable.innerHTML = '<thead><tr><th>Image</th><th>Name</th><th>Email</th><th>Status</th></tr></thead>';
    const maleTableBody = document.createElement('tbody');
    renderTable(maleStudents, maleTableBody);
    maleTable.appendChild(maleTableBody);
    
    genderTableContainer.appendChild(femaleTable);
    genderTableContainer.appendChild(maleTable);
    
    // Append the gender tables below the main table
    document.body.appendChild(genderTableContainer);
});
